...
def get_data(url_page: str, sp_topic: str) -> list:
    ...
    url_analyzed, url_visited = extract_data_from_content_page(url_page, url_visited, url_analyzed, sp_topic)
...
def get_page_code(url_web: str) -> BeautifulSoup:
    """
    Obtener codigo HTML de la pagina
    :param url_web: url of the web to get the HTML code
    """
...
def extract_data_from_content_page(url_page: str, url_visited: list, url_analyzed: pd.DataFrame, sp_topic:str) -> tuple:
    """
    Extrae los datos desde el contenido de la pagina web
    :param url_page (str): direccion web de la pagina
    :param url_visited (list): lista de las url ya visitadas
    :param url_analyzed (DataFrame): DataFrame con todos los datos extraidos del sitio
    :param sp_topic (str): Categoria de la pagina web (salud, deporte, etc)
    """
    ...
    # url not visited
    t = [item for item in t if 'cuba/' in item or 'salud/' in item]
    url_not = list(set(t).difference(url_visited))
    if len(url_not) > 0:
        for url_ in url_not:
            if 'cuba/' in url_ or 'salud/' in url_ and '#comentarios' not in url_:
                if 'http://' not in url_:
...
if __name__ == '__main__':
    ...
    for topic in GR_pages:
        if topic not in topic_visited:
            get_data(url,  topic)
        pd.DataFrame(['topic']).to_csv('GR_topics.csv', headear=False)
